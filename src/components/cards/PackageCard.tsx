...
export const PackageCard: React.FC<IProps> = ({ type }) => {
  ...
  return (
    <>
      <div className="flex-grow-1 request-info-wrapper mb-3">
        <Row
          style={size.width < 768 ? { width: "318px", margin: "auto" } : null}
        >
          {loading
            ? (data?.items ?? []).map((item) => (
